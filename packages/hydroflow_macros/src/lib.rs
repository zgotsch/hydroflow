...
use quote::quote;
use syn::{parse_macro_input, DeriveInput, Ident, Data, DataStruct, Fields, Meta};
use proc_macro_crate::{crate_name, FoundCrate};

#[proc_macro_derive(LatticeRepr, attributes(lr_ignore))]
...
fn derive_latticerepr_internal(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let phantom_tuple_items = generics.type_params()
        .map(|t| {
            let generic_ident = &t.ident;
            quote! { #generic_ident }
        })
        .chain(generics.lifetimes()
            .map(|l| {
                let lt = &l.lifetime;
                quote! { & #lt () }
            })
    ...
    let expanded = quote! {
        ...
        #vis struct #name_lattice #impl_generics #where_clause {
            _phantom: std::marker::PhantomData<( #(#phantom_tuple_items),* )>,
        }
        #[automatically_derived]
